...
/**
 * Treat the "master" and "core" brand the same.
 * "master" was renamed "core"
 *
 * @param {Array[Strings]} values
 * @returns
 */
function addBrandSynonyms(values) {
    if(values.includes('master')) {
        values.push('core');
    }
...
module.exports = (property, value) => {
    let values = (value && typeof value === 'string' ? value.split(',') : []);
    if(property === 'brands') {
        values = addBrandSynonyms(values);
    }
    return (model) => {
