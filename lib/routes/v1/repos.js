...
module.exports = app => {
	...
	app.get('/v1/repos/:repoId/bundles/:bundleType', requireAuth(requireAuth.READ), cacheForFiveMinutes, async (request, response, next) => {
		try {
			// TODO, bundle type sanitation
			// TODO, totally refactor
			const versions = await app.model.Version.fetchByRepoId(request.repo.get('repo_id'));
			const bundlesPromises = versions.map(async version => {
				let versionBundles = await app.model.Bundle.fetchByVersionId(version.id, request.params.bundleType);
				if (!versionBundles || versionBundles.length === 0) {
					versionBundles = await app.model.Bundle.create(version, request.params.bundleType);
				}
				return versionBundles;
			});
			response.send({
				'bundleType': request.params.bundleType,
				'bundles': await Promise.all(bundlesPromises),
			});
		} catch (error) {
			next(error);
		}
