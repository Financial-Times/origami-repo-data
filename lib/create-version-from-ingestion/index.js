 ...
 * @returns void
 */
 async function createVersionFromNpmIngestion(ingestion, Version, npmRegistryUrl) {
	try {
		...
		await downloadAndUnpackPackage(location, packageName, tag, npmRegistryUrl);
		...
		let url = `https://github.com/Financial-Times/${name}`;
		if (packageManifest && packageManifest.repository && packageManifest.repository.url) {
			// Remove the git+ from the start of the url. E.G. "git+https://github.com/Financial-Times/o-autocomplete.git"
			url = packageManifest.repository.url.replace('git+', '');
		}
 ...
 * @returns void
 */
module.exports = async function createVersionFromIngestion(ingestion, version, githubClient, npmRegistryUrl) {
	try {
		...
		if (ingestion.get('type') === 'version') {
			...
			return createVersionFromNpmIngestion(ingestion, version, npmRegistryUrl);
		} else {
