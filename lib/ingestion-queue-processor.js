...
module.exports = class IngestionQueueProcessor {
	...
	async handleIngestionError(ingestion, error) {
		// Associate the ingestion with the error.
		if (ingestion) {
			error.ingestion = {
				url: ingestion.get('url'),
				tag: ingestion.get('tag'),
				type: ingestion.get('type')
			};
		}
		...
		// Destroy an unrecoverable ingestion and report.
		if (ingestion && error.isRecoverable !== true) {
			await ingestion.destroy();
