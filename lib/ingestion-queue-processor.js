...
module.exports = class IngestionQueueProcessor {
	...
	async processIngestions() {
		...
		try {
			...
			if (ingestion) {
				...
				if (ingestion.get('type') === 'bundle') {
					...
					const bundles = await this.Bundle.createBundlesForVersion(version);
					this.logIngestionProgress(ingestion, {
						type: 'success',
		...
		setTimeout(this.processIngestions, pollInterval);
	}
